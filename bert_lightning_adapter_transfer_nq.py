...
def get_dataloader():
    ...
    # train = nq  # ConcatDataset([train, nq])
...
if __name__ == "__main__":
    ...
    # NUM_EPOCHS = 3
    # END
    learning_rate_options = [1e-1]
    adapter_size_options = [8]
    num_epochs_options = [20]

    load_learning_rate = 0.1
    load_adapter_size = 8
    load_num_epochs = 30
    load_saved_epoch = 29

    load_name = model_name + "_" + str(load_learning_rate) + "_" + str(BATCH_SIZE) + "_adapter_" + str(load_adapter_size) + "_epochs_" + str(load_num_epochs) + "_SQuAD_linear_schedule"
    load_ckpt_directory = "./" + load_name + "/"
    load_save_prefix = load_name + "_"

    for NUM_EPOCHS in num_epochs_options:
        for LEARNING_RATE in learning_rate_options:
            for ADAPTER_SIZE in adapter_size_options:

                name = model_name + "_" + str(LEARNING_RATE) + "_" + str(BATCH_SIZE) + "_adapter_" + str(ADAPTER_SIZE) + "_epochs_" + str(NUM_EPOCHS) + "_SQuAD_transfer_NQ_" + str(long_candidates_per_example) + "-cands-per-example"
                ckpt_directory = "./" + name + "/"
                SAVE_PREFIX = name + "_"
                ...
                print("loading SQuAD pretrained weights...")
                checkpoint = torch.load(load_ckpt_directory + load_save_prefix + "_ckpt_epoch_" + str(load_saved_epoch) + ".ckpt", map_location=lambda storage, loc: storage)
